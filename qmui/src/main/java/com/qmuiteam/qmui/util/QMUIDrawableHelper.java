...
public class QMUIDrawableHelper {
    ...
    public static Bitmap createBitmapFromView(View view, float scale) {
        ...
        view.clearFocus();
        Bitmap bitmap = createBitmapSafely((int) (view.getWidth() * scale), (int) (
                view.getHeight() * scale), Bitmap.Config.ARGB_8888, 1);
        if (bitmap != null) {
    ...
    public static Bitmap createBitmapFromView(View view, int leftCrop, int topCrop, int rightCrop, int bottomCrop) {
        ...
        Bitmap cutBitmap = createBitmapSafely(
                view.getWidth() - rightCrop - leftCrop,
                view.getHeight() - topCrop - bottomCrop, Bitmap.Config.ARGB_8888, 1);
        if (cutBitmap == null) {
            return null;
        }
        Canvas canvas = new Canvas(cutBitmap);
        Rect src = new Rect(leftCrop, topCrop,
                view.getWidth() - rightCrop, view.getHeight() - bottomCrop);
        Rect dest = new Rect(0, 0,
                view.getWidth() - rightCrop - leftCrop, view.getHeight() - topCrop - bottomCrop);
        canvas.drawColor(Color.WHITE); // 防止 View 上面有些区域空白导致最终 Bitmap 上有些区域变黑
    ...
    public static ColorFilter setDrawableTintColor(Drawable drawable, @ColorInt int tintColor) {
        LightingColorFilter colorFilter = new LightingColorFilter(Color
                .argb(255, 0, 0, 0), tintColor);
     ...
     */
    @TargetApi(16)
    public static GradientDrawable createCircleGradientDrawable(@ColorInt int startColor, @ColorInt int endColor, int radius, @FloatRange(from = 0f, to = 1f) float centerX, @FloatRange(from = 0f, to = 1f) float centerY) {
     ...
     */
    public static LayerDrawable createItemSeparatorBg(@ColorInt int separatorColor, @DrawableRes int bgColor, int separatorHeight, boolean top) {
    ...
    public static LayerDrawable createItemSeparatorBg(@ColorInt int separatorColor, @DrawableRes int bgColor, int separatorHeight, boolean top, Context context) {
        ...
        //bg.getPaint().setStyle(Paint.Style.FILL);
        //bg.getPaint().setColor(bgColor);
        Drawable drawable = getVectorDrawable(context, bgColor);

        Drawable[] layers = { separator, drawable };
        LayerDrawable layerDrawable = new LayerDrawable(layers);
    ...
    Drawable getVectorDrawable(Context context, @DrawableRes int resVector) {
        try {
            ...
            QMUILog.d(TAG, "Error in getVectorDrawable. resVector=" + resVector + ", resName=" +
    ...
    public static Bitmap vectorDrawableToBitmap(Context context, @DrawableRes int resVector) {
        ...
        if (drawable != null) {
            Bitmap b = Bitmap.createBitmap(drawable.getIntrinsicWidth(), drawable
